...
recover<-PAM%>%filter(DEPLOYSTATUS == "Recovery")%>%as.data.frame()%>%
  dplyr::select("Recover_Datetime_UTC" = "CaptureDate", "Recover_LAT" = "LAT_DD", "Recover_LON" = "LONG_DD", PAMID, "Recover_notes" = "NOTES")

all_deploy_data<-PAM%>%filter(DEPLOYSTATUS == "Deployment")%>%dplyr::select(PAMID, "Deploy_Datetime_UTC" = "CaptureDate", "Deploy_LAT" = "LAT_DD", "Deploy_LON" = "LONG_DD", DEPTHFT, "Deploy_notes" = "NOTES")%>%
  ...
  as.data.frame()%>%
  arrange(Deploy_Datetime_UTC)%>%
  dplyr::select(PAMID, SAT_beacon_ID, SoundTrap_ID,VR2_ID, Deploy_Datetime_UTC, Deploy_LAT, Deploy_LON, DEPTHFT, `Pre-deploy_notes`, Deploy_notes, Recover_Datetime_UTC, Recover_LAT, Recover_LON, Recover_notes)

max_deploy_date<-max(as.Date(all_deploy_data$Deploy_Datetime_UTC))
