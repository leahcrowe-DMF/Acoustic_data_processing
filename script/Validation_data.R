...
ggplot(Mnsong)+
  geom_point(aes(x = date, y = Site, color = confidence))+
...
detection_bin<-analysis_data_df%>%
  ...
  group_by(Site, date, tod_bin, validated_sp, confidence)%>%
  mutate(n = n())%>%
  mutate(confidence2 = case_when(
    confidence == "Definite" & n >= 3 ~ "3+ calls",
    TRUE ~ "<3 calls"
  ))%>%
  distinct(Site, date, tod_bin, validated_sp, confidence2)%>%
  arrange(Site, date, tod_bin, validated_sp, confidence2)%>%
  group_by(Site, date, tod_bin, validated_sp)%>%
...
recording_dead_periods<-ST_recording%>%
  ...
  mutate(Site = substr(Deployment_ID, 1, 5))%>%
  filter(Site != "")%>%
...
detection_bin%>%filter(validated_sp == "Right whale" & confidence2 == "Possible")%>%filter(Site == "EOS08")%>%ungroup()%>%distinct(date, tod_bin)

detection_bin%>%filter(is.na(tod_bin))

detection_bin%>%filter(is.na(Site))
...
detection_bin%>%filter(validated_sp == "Humpback whale")%>%filter(Site == "BUZ17")
